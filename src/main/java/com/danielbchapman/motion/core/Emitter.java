...
public abstract class Emitter<T extends Point>
{
   ...
   * @param randomTime
   */
  public Emitter(VerletPhysics3D physics,Vec3D position, Vec3D heading, int lifeSpan, int rate, float randomVector, int randomTime)
  {
...
//      System.out.println("[SKIPPED] Calling update" + time + ", " + lastTime + " " + nextDelta);
  ...
  public void addPoint(long time)
  {
    ...
    while(it.hasNext())
    {
      Point px = it.next(); //This should probably implement a "fade-able" interface
      if(px != null)
