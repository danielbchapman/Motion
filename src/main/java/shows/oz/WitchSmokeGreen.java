...
public class WitchSmokeGreen extends BaseScene
{
  DwFluid2D fluid;
  ...
  { 
    testKeys.put(new KeyCombo(' '), (m)->{
      motion.println("This calls the outer motion on go and is this->" + WitchSmokeGreen.this);
    } );
    ...
    fluid.addCallback_FluiData(new DwFluid2D.FluidData(){
      public void update(DwFluid2D fluid){
        for(MotionMouseEvent e : eventsSinceUpdate)
        {
          ...
          if(e.left)
          {
            fluid.addDensity(px, py, 20, 0f, 1f, 0f, 0f);
            fluid.addDensity(px, py, 8, .5f, .5f, .5f, 1.0f);
