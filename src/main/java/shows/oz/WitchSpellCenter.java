...
public class WitchSpellCenter extends BaseScene
{
  DwFluid2D fluid;
  ...
  ArrayList<ArrayList<RecordAction2017>> events;

  @Override
  public void initialize(Motion motion)
  {
    events = new ArrayList<>();
    events.add(Recorder2017.load("captures/witch-spell-figure8", motion.WIDTH, motion.HEIGHT, 0, 0));
    events.add(Recorder2017.load("captures/center-spell-accent", motion.WIDTH, motion.HEIGHT, 0, 0));
    events.add(Recorder2017.load("captures/center-accent-2", motion.WIDTH, motion.HEIGHT, 0, 0));
  ...
  {
    ArrayList<RecordAction2017> spells = events.get(lastGo % events.size());
    motion.runPlayback("witch-flyby", spells, new MouseBrush());
    lastGo++;
  }
